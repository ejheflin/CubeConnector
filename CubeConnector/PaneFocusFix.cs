...
namespace CubeConnector
{
    ...
    /// over focus events: when an Excel grid window (class "EXCEL7") gains focus while keyboard
    /// focus is still stuck on our WebView2 subtree, we push focus back to the grid. The hook is
    /// scoped to this process and active only while installed; inert otherwise.
    /// </summary>
    internal static class PaneFocusFix
    {
            ...
            int idObject, int idChild, uint dwEventThread, uint dwmsEventTime)
        {
            ...
            try
            {
                if (GetClass(hwnd) != "EXCEL7") return;   // only react to the grid gaining focus
